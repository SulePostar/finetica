...
const { KufProcessedFile } = require('../models');
...
class FileTracker {
    ...
    static async getUnprocessedFiles(bucketFiles, forceReprocess = false) {
        try {
            Logger.info('Checking for unprocessed files in kuf_processed_files table...');
            ...
            if (forceReprocess) {
                ...
                const allTrackedFiles = await KufProcessedFile.findAll({
                    where: {
                        fileName: fileNames
                    },
            ...
            const unprocessedTrackedFiles = await KufProcessedFile.findAll({
                where: {
                    fileName: fileNames,
                    processed: false
                },
            ...
            const processedTrackedFiles = await KufProcessedFile.findAll({
                where: {
                    fileName: fileNames,
                    processed: true
                },
    ...
    static async trackFileProcessing(fileName, forceReprocess = false) {
        try {
            // Find existing KUF processed file record (don't create new ones)
            const record = await KufProcessedFile.findOne({
                where: { fileName }
            });

            // If no record exists, skip this file (someone else should have created it)
            if (!record) {
                Logger.warn(`File ${fileName} not found in kuf_processed_files table, skipping...`);
                return {
                    record: null,
                    shouldSkip: true,
    ...
    static async resetAllProcessing() {
        try {
            Logger.info('Resetting processing status for all KUF files...');

            const result = await KufProcessedFile.update(
                {
                    processed: false,
                    processedAt: null,
                    errorMessage: null
    ...
    static async getProcessingStatus() {
        try {
            const totalFiles = await KufProcessedFile.count();
            const processedFiles = await KufProcessedFile.count({ where: { processed: true } });
            const failedFiles = await KufProcessedFile.count({
                where: {
                    processed: false,
    ...
    static async healthCheck() {
        try {
            await KufProcessedFile.findOne({ limit: 1 });
            return true;
        } catch (error) {
