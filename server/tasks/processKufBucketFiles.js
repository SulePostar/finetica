...
class KufBucketProcessingCLI {
    constructor() {
        ...
        this.kufProcessor = new KufProcessor(this.config);
    ...
    /**
     * List all files in the KUF bucket
     */
    ...
    /**
     * Process a single KUF file with tracking
     */
    async processKufFile(file, options = {}) {
        try {
            ...
            // Process the file
            const result = await this.kufProcessor.processFile(file, options);

            // Update tracking based on result
            if (result.success) {
                await FileTracker.markAsProcessed(tracking.record);
                result.kufProcessedId = tracking.record.id;
            } else {
            ...
            Logger.error(`Error in KUF file processing pipeline: ${error.message}`);
            return {
                success: false,
                fileName: file.name,
                error: error.message,
    ...
    /**
     * Process all unprocessed KUF files
     */
    async processAllFiles(options = {}) {
        ...
        try {
            Logger.info('Starting KUF bucket file processing...');
            ...
            if (bucketFiles.length === 0) {
                Logger.info('No PDF files found in KUF bucket');
            ...
            if (unprocessedFiles.length === 0) {
                Logger.info('All files in KUF bucket have already been processed');
            ...
            for (let i = 0; i < filesToProcess.length; i++) {
                ...
                try {
                    ...
                    const result = await this.processKufFile(file, {
                        userId,
            ...
            Logger.success(`KUF processing completed: ${summary.processed} processed, ${summary.errors} errors, ${summary.skipped} skipped in ${summary.duration}`);

            return summary;

        } catch (error) {
            Logger.error(`KUF processing failed: ${error.message}`);
            throw error;
        }
    }
    ...
    async healthCheck() {
        try {
            ...
            const processorChecks = await this.kufProcessor.healthCheck();
...
module.exports = KufBucketProcessingCLI;

// Run CLI if this file is executed directly
if (require.main === module) {
    const cli = new KufBucketProcessingCLI();
    cli.run();
};